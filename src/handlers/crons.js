...
async function notificarPendientesConHorarios(disponibilidadSemana, clienteState) {
  ...
  for (const p of pendientes) {
    ...
    if (dias.length > 1) {
      ...
      clienteState[p.telefono] = { paso: 'eligiendo_dia', dias, nombre: p.nombre };
      await sendMessage(destino,
      ...
      clienteState[p.telefono] = { paso: 'eligiendo_hora', dia: dias[0].label, fecha: dias[0].fecha, slots, nombre: p.nombre };
      await sendMessage(destino,
        `✂️ ¡Hola *${primerNombre(p.nombre)}*! *Saviac Estilo* confirmó horarios para este *${dias[0].label.toLowerCase()}* 💈\n\nEstos son los horarios disponibles:\n\n${lista}\n\n¿Cuál te queda mejor?\n\n_Responde con el número de tu opción_`
      );
    }
...
async function enviarRecordatorio15Min(clienteState) {
  ...
  const citasHoy = citas.filter(c =>
    c.fecha === hoy && c.estado === 'confirmada' && c.recordatorio_enviado !== 'TRUE'
  );
